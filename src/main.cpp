...
void loadConfig() {
  ...
  config.altitude = prefs.getFloat("alt", DEFAULT_ALTITUDE);
...
void showSplash() {
  ...
  setDisplayContent("LoRa APRS WX PRO by IU1VDW",
...
String buildAPRS() {
  ...
  // 1. Calcoli temperatura e coordinate (invariati)
  ...
  // 2. COMPENSAZIONE PRESSIONE (QFF)
  // Usiamo la formula barometrica internazionale:
  // P_sl = P_stazione * (1 - (0.0065 * alt) / (T_celsius + 0.0065 * alt + 273.15)) ^ -5.257
  float pCompensated = P * pow(1.0 - (0.0065 * config.altitude) / (T + 0.0065 * config.altitude + 273.15), -5.257);

  // 3. Preparazione dati per la stringa APRS
  ...
  // Usiamo pCompensated invece di P
  int pressureInt = (int)(pCompensated * 10); 

  // 4. Costruzione stringa finale
  sprintf(buffer,
